 ...
 * 支持手势缩放和平移的ImageView
 */
public class ZoomTranslateImageView extends AppCompatImageView implements ScaleGestureDetector.OnScaleGestureListener {
    ...
    private float minScale = 0.9f;//最小缩放到原图的0.5倍
    private float lastX=0, lastY=0;
    private int lastPointerCount;

    public ZoomTranslateImageView(Context context) {
        super(context);
        init(context);
    }

    public ZoomTranslateImageView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ZoomTranslateImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context) {
        ...
        scaleGestureDetector = new ScaleGestureDetector(new WeakReference<Context>(context).get(),
                new WeakReference<ZoomTranslateImageView>(this).get());
        mMatrix = new Matrix();
    }
